...
void can_init(CanInit init, uint8_t rxInterrupt){
    ...
    CAN0->CAN_BR = init.reg; /**< \brief (Can Offset: 0x0014) Baudrate Register */
...
uint8_t can_rx(CanMsg* m){
    if(!(CAN0->CAN_MB[rxMailbox].CAN_MSR & CAN_MSR_MRDY)){
        return 0;/**< \brief (Can Offset: 0x0014) Baudrate Register */
    }
...
void can_init_super_viktig_riktig_ikke_touch_for_faen(){
    can_init((CanInit){.brp = 20, .phase1 = (6-1), .phase2 = (6-1), .propag = (4-1), .sjw = (2-1), .smp = 0}, 0);
...
void CAN0_Handler(void){
    ...
    if(can_sr & CAN_SR_MB0){
        ...
        //< \brief (Can Offset: 0x0014) Baudrate Register 
    }
